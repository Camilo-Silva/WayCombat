...
export const handler: Handler = async (event, context) => {
  ...
  try {
    ...
    // Verificar que el mix existe y obtener su estado actual
    const mixCheck = await sql`
      SELECT id, titulo, activo FROM mixes WHERE id = ${mixId}
    `;
    ...
    const currentMix = mixCheck[0];
    const newActiveState = !currentMix.activo;

    // Actualizar el estado activo del mix
    await sql`
      UPDATE mixes 
    ...
    // Si se desactiva el mix, remover todos los accesos
    if (!newActiveState) {
      await sql`
    ...
    return {
      ...
      headers,
      body: JSON.stringify({ 
        message: `Mix ${newActiveState ? 'activado' : 'desactivado'} correctamente`,
        mixId: mixId,
        titulo: currentMix.titulo,
